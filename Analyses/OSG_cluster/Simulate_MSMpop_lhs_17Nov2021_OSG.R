...
if(file.exists("data/sim1/sim1.cp.rda") == FALSE){
  ...
  params2dim <- readRDS(system.file("data/params2dim_17Nov2021.RDS",
  ...
  if(n_pop1 > 10000){
    ...
    est <- netest(nw, formation, target.stats, coef.diss, edapprox = TRUE)
    dx <- netdx(est, nsims = 1, nsteps = 5000, keep.tedgelist = TRUE)
...
sim <- netsim_hpc("est.rda", param, init, control,
                  cp.save.int = 100, save.min=FALSE, save.max=FALSE,
                  compress = "gzip")

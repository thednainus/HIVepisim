...
if(file.exists("data/sim1/sim1.cp.rda") == FALSE){
  ...
  params3dim <- readRDS(system.file("data/params3dim_18Nov2021.RDS",
  ...
  #n_inf_pop1 <- 300.9307
  n_inf_pop1 <- params3dim$init_pop1_param
  ...
  save(params3dim, time.unit, departure_rate_years, dr_vec, file = "params.rda")
}
...
inf.prob.param <-  readRDS(system.file("data/transmission_probability.RDS",
                                       package = "HIVepisim"))
inf.prob.param$trans.prob <- inf.prob.param$trans.prob * params3dim$multiplier
act.rate.param <- 0.22
...
param <- param.net(time.unit = time.unit,
                   ...
                   art_start = art_start,
                   tx.init.prob = params3dim$tx.init.prob.param,
                   tx.halt.prob = 0,
                   tx.reinit.prob = 0,

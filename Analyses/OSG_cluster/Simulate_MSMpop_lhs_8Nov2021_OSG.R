...
print(file.exists("data/sim1/sim1.cp.rda"))
...
if(file.exists("data/sim1/sim1.cp.rda") == FALSE){
  ...
  #params <- sampler2(n=1000, paramdf = data.frame(init_pop1_param = c(1, 10),
  #                                               tx.init.prob.param = c(0.0001, 0.01253765)))

  #saveRDS(params, "inst/data/params2dim_5Nov2021.RDS")
  params2dim <- readRDS(system.file("data/params2dim_5Nov2021.RDS",
  ...
  #n_inf_pop1 <- 300.9307
  n_inf_pop1 <- params2dim$init_pop1_param
  ...
  target.stats <- c((2.38 * n_pop1/2 + 2.38 * n_pop2/2), 2.38 * n_pop2/2, 0)
  ...
  save(params2dim, time.unit, departure_rate_years, dr_vec, file = "params.rda")
}
...
param <- param.net(time.unit = time.unit,
                   ...
                   art_start = art_start,
                   tx.init.prob = params2dim$tx.init.prob.param,
                   tx.halt.prob = 0,
                   tx.reinit.prob = 0,
...
control <- control.net(type = NULL, simno = 1, currsim = 1,
                       nsteps = total_steps, start = 1, nsims = 1,
...
sim <- netsim_hpc("est.rda", param, init, control,
                  cp.save.int = 10000, save.min=FALSE, save.max=FALSE,
                  compress = "gzip")
